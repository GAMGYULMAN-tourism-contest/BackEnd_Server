...
public class ScheduleController {
    ...
    public CustomResponse<ScheduleResponseDTO.CreatedScheduleDTO> createSchedule(@AuthenticatedMember Member member,
        ...
        return CustomResponse.onSuccess(scheduleCommandService.createSchedule(member, dto));
    }
    ...
    public CustomResponse<ScheduleResponseDTO.UpdatedScheduleDTO> updateSchedule(@AuthenticatedMember Member member,
        ...
        return CustomResponse.onSuccess(scheduleCommandService.updateSchedule(member, scheduleId, dto));
    }

    @DeleteMapping("/{scheduleId}")
    @Operation(summary = "스케줄 삭제하기", description = "스케줄을 삭제하는 API, 삭제는 주인(OWNER)만 가능")
    ...
    public CustomResponse<ScheduleResponseDTO.ScheduleInfoDTO> getSchedule(@PathVariable("scheduleId") Long scheduleId) {
        Schedule schedule = scheduleQueryService.getSchedule(scheduleId);
        return CustomResponse.onSuccess(ScheduleResponseDTO.ScheduleInfoDTO.from(schedule));
    }
}
